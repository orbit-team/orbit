...
const char * const BITCOIN_CONF_FILENAME = "orbit.conf";
const char * const BITCOIN_PID_FILENAME = "orbitd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "orbit" is a composite category enabling all Orbit-related debug output
            if(ptrCategory->count(string("orbit"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OrbitCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OrbitCore
    // Mac: ~/Library/Application Support/OrbitCore
    // Unix: ~/.orbitcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OrbitCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OrbitCore";
#else
    // Unix
    return pathRet / ".orbitcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty orbit.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override orbit.conf
